...
@app.route("https://chemical-calculator.vercel.app/api/chemCalc", methods = ['POST','OPTIONS'])
@cross_origin()
def disp():
     body=request.json
     print(body)
...
def eveluateSubReactionsAdd(reactions,x,y):
    ...
    historyTabel = pd.DataFrame(columns =["reaction","NumberOfIterations",Xlable,Ylable,Glable,X1lable,X2lable,Xablable,Gablable,X1ablable,X2ablable,Zlable])
    historyTabel = historyTabel.append ({"reaction":"Intial Values","NumberOfIterations" : 0,Xlable : X,Ylable : Y,Glable:G,X1lable : X1,X2lable :X2,Xablable:Xab,Gablable:Gab,X1ablable:X1ab,X2ablable:X2ab,Zlable:Z}, ignore_index = True)

    while y>0 : 
      eveluateReactionsAdd(reactions,historyTabel,X,y) 
      y = y - 1 


    return Z,historyTabel   
...
def eveluateSubReactionsSub(reactions,x,y):
    ...
    while y>0 : 
      eveluateReactionsSub(reactions,historyTabel,X,y) 
      y = y - 1 
...
def Division(x,y):
  ...
  while(x>=y):
    x =  evaluateDivisionSub(x,y,historyTabel,reactions)
    quotient=evaluateDivisionAdd(quotient,1,historyTabel,reactions)
  ...
  return s,reactions,quotient,historyTabel.values.tolist(),varDivision,historyTabel
...
def eveluateSubReactionsMul(reactions,x,y):
    ...
    historyTabel = pd.DataFrame(columns =["reaction","NumberOfIterations",Xlable,Ylable,ilable,Y1lable,Zlable])
    historyTabel = historyTabel.append ({"reaction":"Intial Values","NumberOfIterations" : 0,Xlable : X,Ylable : Y,ilable:I,Y1lable:Y1,Zlable:Z}, ignore_index = True)

    
    while x>0 : 
      eveluateReactionsMul(reactions,historyTabel,X,Y) 
      x = x - 1 
